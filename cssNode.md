...
### 基本选择器扩展
#### 直接后代选择器 >
        #div1 > .inner {...}   只能选择自己后1代的元素
#### 直接相邻兄弟选择器 +
        #div1 .first + .second {...} 
            匹配一个 在.first元素 之后的 且class为.second的兄弟元素
#### 通用兄弟选择器 ~
        #div1  .first ~ .second {...} 匹配所有 在.first元素 之后 且class为.second的元素
...
* [name~="2333"]{...}
*     匹配所有 以空格分隔的name属性值 列表中 含有 2333 的元素
*     <div name="2333 2444"></div>
*  .div1[name] {...} 
...
#### CSS声明的优先级
    ...
    1,0,0,0 为最高特殊性 ，若选择器产生冲突，以最高特殊性选择器来渲染
    特殊性可叠加，但不产生进位 例如 0,100,0,0
...
> 注意 ：  
    如果父级元素的宽度小于了 子元素的宽度 ，子元素的宽度会被压缩成 父元素的宽度
* flex-flow
